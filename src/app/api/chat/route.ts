...
// We check GRADIENT_AGENT_KEY first, falling back to GRADIENT_MODEL_ACCESS_KEY for backwards compatibility.
const AGENT_ACCESS_KEY = process.env.GRADIENT_AGENT_KEY || process.env.GRADIENT_MODEL_ACCESS_KEY || "";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!AGENT_ACCESS_KEY) {
      return NextResponse.json({
        results: [{
          agent: "System",
          key: "error",
          content: "⚠️ **Agent Access Key not set.** Add `GRADIENT_AGENT_KEY` to `.env.local`.",
        }],
      });
    }
    ...
    const agentPromises = AGENTS.map(async (agent, i) => {
      ...
      const content = await fetchDOAgent(agent.url, compiledUserContext);
